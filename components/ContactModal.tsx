...
const DeliverableRow: React.FC<{ label: string; agreed: boolean; delivered: boolean; onAgreedChange: (value: boolean) => void; onDeliveredChange: (value: boolean) => void; }> = ({ label, agreed, delivered, onAgreedChange, onDeliveredChange }) => ( <div className="flex justify-between items-center p-3 bg-accent rounded-md"> <span className="font-medium text-white text-sm">{label}</span> <div className="flex items-center space-x-6"> <label className="flex items-center space-x-2 cursor-pointer text-xs"><input type="checkbox" checked={agreed} onChange={e => onAgreedChange(e.target.checked)} className="form-checkbox bg-primary text-highlight focus:ring-highlight" /><span>Agreed</span></label> <label className="flex items-center space-x-2 cursor-pointer text-xs"><input type="checkbox" checked={delivered} onChange={e => onDeliveredChange(e.target.checked)} disabled={!agreed} className="form-checkbox bg-primary text-highlight focus:ring-highlight disabled:opacity-50" /><span>Delivered</span></label> </div> </div> );

export const ContactModal: React.FC<ContactModalProps> = ({ contact, onClose, onUpdate, onAddInteraction, onSyncGmail, isGmailConnected, settings, tasks, onTaskAdd, onTaskUpdate, onTaskDelete }) => {
  ...
  useEffect(() => { if (activeTab === 'partnership' && contact.partnershipType !== PartnershipType.PARTNER) { setActiveTab('interactions'); } else if (contact.partnershipType === PartnershipType.PARTNER) { setActiveTab('partnership'); } }, [contact.id, contact.partnershipType, activeTab]);

  const handleUpdate = (updatedData: Partial<Contact>) => { const updatedContact = { ...editableContact, ...updatedData }; setEditableContact(updatedContact); onUpdate(updatedContact); };
  const handlePartnerDetailChange = (field: keyof PartnerDetails, value: any) => { handleUpdate({ partnerDetails: { ...editableContact.partnerDetails!, [field]: value } }); };
  ...
  const handleFieldChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    handleUpdate({ [name]: isNumeric ? (value ? parseInt(value, 10) : undefined) : value });
  };
  ...
  const renderTabs = () => (
    <div className="md:col-span-2 flex flex-col">
        <div className="border-b border-accent -mt-2">
            ...
            {contact.partnershipType === PartnershipType.PARTNER && <TabButton active={activeTab === 'partnership'} onClick={() => setActiveTab('partnership')}>Partnership Details</TabButton>}
  ...
  const renderPartnershipDetails = () => ( <div className="space-y-4 overflow-y-auto pr-2 mt-4 flex-grow bg-primary p-3 rounded-md"> {editableContact.partnerDetails && (<> <div className="grid grid-cols-2 gap-4"><label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" checked={editableContact.partnerDetails.contractSigned} onChange={e => handlePartnerDetailChange('contractSigned', e.target.checked)} className="form-checkbox bg-accent text-highlight focus:ring-highlight" /><span>Contract Signed</span></label><label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" checked={editableContact.partnerDetails.continueFollowUp} onChange={e => handlePartnerDetailChange('continueFollowUp', e.target.checked)} className="form-checkbox bg-accent text-highlight focus:ring-highlight" /><span>Continue Follow-ups</span></label></div> <div className="border-t border-accent pt-4"><h5 className="font-semibold text-white mb-2">Deliverables</h5><div className="space-y-3"><div className="p-3 bg-accent rounded-md"><div className="flex justify-between items-center"><span className="font-medium text-white text-sm">Drill Videos:</span><div className="flex items-center space-x-2"><input type="number" value={editableContact.partnerDetails.drillVideosDelivered} onChange={e => handlePartnerDetailChange('drillVideosDelivered', Math.max(0, parseInt(e.target.value)))} className="w-16 bg-primary p-1 rounded text-center" /><span className="text-text-secondary">/</span><input type="number" value={editableContact.partnerDetails.drillVideosAgreed} onChange={e => handlePartnerDetailChange('drillVideosAgreed', Math.max(0, parseInt(e.target.value)))} className="w-16 bg-primary p-1 rounded text-center" /></div></div></div><DeliverableRow label="Testimonial Video" agreed={editableContact.partnerDetails.testimonialVideoAgreed} delivered={editableContact.partnerDetails.testimonialVideoDelivered} onAgreedChange={val => handlePartnerDetailChange('testimonialVideoAgreed', val)} onDeliveredChange={val => handlePartnerDetailChange('testimonialVideoDelivered', val)} /><DeliverableRow label="Website Link" agreed={editableContact.partnerDetails.websiteLinkAgreed} delivered={editableContact.partnerDetails.websiteLinkDelivered} onAgreedChange={val => handlePartnerDetailChange('websiteLinkAgreed', val)} onDeliveredChange={val => handlePartnerDetailChange('websiteLinkDelivered', val)} /><DeliverableRow label="Social Media Post" agreed={editableContact.partnerDetails.socialPostAgreed} delivered={editableContact.partnerDetails.socialPostDelivered} onAgreedChange={val => handlePartnerDetailChange('socialPostAgreed', val)} onDeliveredChange={val => handlePartnerDetailChange('socialPostDelivered', val)} /></div></div> <div className="border-t border-accent pt-4"><h5 className="font-semibold text-white mb-2">Deliverable Links</h5><input type="text" placeholder="Testimonial Video URL" value={editableContact.partnerDetails.testimonialVideoLink} onChange={e => handlePartnerDetailChange('testimonialVideoLink', e.target.value)} className="w-full bg-accent p-2 rounded mb-2 text-sm" /><input type="text" placeholder="Website Link URL" value={editableContact.partnerDetails.websiteLinkUrl} onChange={e => handlePartnerDetailChange('websiteLinkUrl', e.target.value)} className="w-full bg-accent p-2 rounded mb-2 text-sm" /><input type="text" placeholder="Social Post URL" value={editableContact.partnerDetails.socialPostLink} onChange={e => handlePartnerDetailChange('socialPostLink', e.target.value)} className="w-full bg-accent p-2 rounded text-sm" /></div> </>)}</div> );
  ...
  const EditableField: React.FC<{name: keyof Contact, value: any, label: string, type?: string, icon?: React.ReactNode}> = ({ name, value, label, type='text', icon}) => (
    <div>
        ...
        <div className="relative">
            ...
            <input type={type} name={name} value={value || ''} onChange={handleFieldChange} placeholder={label} className={`w-full bg-accent p-2 ${icon ? 'pl-9' : ''} rounded text-white text-sm`}/>
        </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50 p-4">
      <div className="bg-secondary rounded-lg shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col">
        <div className="p-4 sm:p-6 border-b border-accent flex justify-between items-center"><h2 className="text-2xl font-bold text-white">Contact Details</h2><button onClick={onClose} className="text-text-secondary hover:text-white transition-colors"><CloseIcon /></button></div>
        <div className="flex-grow overflow-y-auto p-4 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1 space-y-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <input type="text" name="name" value={editableContact.name} onChange={handleFieldChange} className="bg-transparent text-xl font-bold text-white w-full"/>
                <select value={editableContact.pipelineStage} onChange={(e) => handleUpdate({ pipelineStage: e.target.value })} className="mt-1 bg-accent text-white text-xs rounded-full px-2 py-1 border-transparent focus:ring-2 focus:ring-white outline-none">{settings.pipelineStages.map(stage => <option key={stage} value={stage}>{stage}</option>)}</select>
              </div>
            </div>
            ...
            <div><h4 className="font-semibold text-white text-sm mb-1">Tags</h4><div className="bg-accent p-2 rounded-md"><div className="flex flex-wrap gap-2 mb-2">{ (editableContact.tags || []).map(tag => ( <span key={tag} className="flex items-center bg-highlight/50 text-blue-200 text-xs px-2 py-1 rounded-full">{tag}<button onClick={() => removeTag(tag)} className="ml-1.5 text-blue-200 hover:text-white"> &times; </button></span> )) }</div><div className="relative"><TagIcon className="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-text-secondary" /><input type="text" value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={handleTagInputKeyDown} placeholder="Add a tag..." className="w-full bg-primary pl-7 p-1 rounded text-white text-sm" /></div></div></div>
            <div><h4 className="font-semibold text-white text-sm mb-1">Next Follow-up Date</h4><input type="date" value={editableContact.nextFollowUpDate ? editableContact.nextFollowUpDate.split('T')[0] : ''} onChange={(e) => handleUpdate({ nextFollowUpDate: e.target.value })} className="w-full bg-accent p-2 rounded text-white text-sm"/></div>
            ...
            <div><h4 className="font-semibold text-white text-sm mb-1">My Notes</h4><textarea value={editableContact.notes || ''} onChange={(e) => handleUpdate({ notes: e.target.value })} placeholder="Add private notes..." className="w-full bg-accent p-2 rounded-md text-white text-sm focus:ring-2 focus:ring-highlight outline-none" rows={3}></textarea></div>
          </div>
          {renderTabs()}
        </div>
      </div>
